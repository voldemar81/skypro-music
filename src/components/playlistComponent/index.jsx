...
import { standingRedact } from './../../data/standingRedact';
import * as S from './styles'

export function PlayList({isLoading, music}) {
  
  const fullPlayList = music.map((item, i) => {
    const { name, author, album, standing_seconds } = item;
      ...
      <S.PlaylistItem key={i}>
        <S.PlaylistTrack>
          ...
          <S.TrackTimeComponent>
            ...
            <S.TrackTimeText>
              {standingRedact(standing_seconds)}
            </S.TrackTimeText>
          </S.TrackTimeComponent>
        </S.PlaylistTrack>
      </S.PlaylistItem>
